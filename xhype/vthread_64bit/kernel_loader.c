...
int load_linux64(const struct virtual_machine* vm, struct vkernel* vkn,
  ...
  uint64_t guest_mem_h = vm_alloc_aligned(mem_size, header->kernel_alignment);
  ...
  bp->e820_map[0].addr = rd_base;
  bp->e820_map[0].size = rd_region_size;
  bp->e820_map[0].type = 1;

  bp->e820_map[1].addr = guest_mem;
  bp->e820_map[1].size = mem_size;
